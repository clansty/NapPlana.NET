...
/// </summary>
public class LifeCycleEventParser(IEventHandler handler) : MetaEventParser(handler)
{
    /// <summary>
    ...
    public override void ParseEvent(string botEvent)
    {
        ...
        handler.BotLifeCycle(lifeCycleEvent);
        switch (lifeCycleEvent.SubType)
        {
            case LifeCycleSubType.Connect:
                handler.LogReceived(LogLevel.Info,$"机器人已连接到NapCat服务器");
                handler.BotConnected();
                break;
            case LifeCycleSubType.Disable:
                handler.LogReceived(LogLevel.Warning,$"机器人已与NapCat服务器断开连接");
                break;
            case LifeCycleSubType.Enable:
                handler.LogReceived(LogLevel.Debug,$"机器人已启用");
                break;
        }
    }
}